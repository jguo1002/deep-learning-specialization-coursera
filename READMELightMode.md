...
>
> <img src="https://render.githubusercontent.com/render/math?math=J^{[l]}_{style}(S, G) = \frac{1}{(2 n^{[l]}_H n^{[l]}_W n^{[l]}_C)^2}    \sum_{i=1}^{n_C}     \sum_{j=1}^{n_C}     (G^{[l](S)}_{(gram) i, j} - G^{[l](G)}_{(gram) i,j})^2 ">
> 
